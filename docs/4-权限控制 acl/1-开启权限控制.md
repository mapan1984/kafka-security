...
Kafka 提供了权限控制的接口(`kafka.security.authorizer.Authorizer`)，并且提供了默认的实现：

- ZK 模式下，用 zookeeper 存储 acl 规则(默认存储路径 `/kafka-acl`)。
- KRaft 模式下，增加 `ClusterMetadataAuthorizer` 接口，使用 `__cluster_metadata` 存储数据
